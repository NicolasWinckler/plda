...
all: lda infer mpi_lda lda_test mpi_lda_test mpi_test

clean:
	rm -rf $(OBJ_PATH)
	rm -f lda mpi_lda infer lda_test mpi_lda_test mpi_test
...
mpi_lda_test: mpi_lda_test.cc $(OBJ)
	$(MPICC) $(CFLAGS) $(OBJ) $< -o $@

mpi_test: mpi_test.cc $(OBJ)
	$(MPICC) $(CFLAGS) $(OBJ) $< -o $@